...
export function MovieList(props:{movies: Movie[], setMovies: Function}):JSX.Element {
    ...
    function deleteMovie(key:string) {
        if (window.confirm('Delete?')) {
            let newArray:Movie[] = [];
            props.movies.find((el:Movie) => { if (el.key !== key) { newArray.push(el); } });
        ...
        {Object.values(props.movies).map((movie) => (
          <Fragment key={movie.key}>
            <MovieCard title={movie.title} description={movie.description} />
              <Button title={'Edit'} type={'button'} onclickAction={() => openModal(movie.key)}/>
              <Button title={'Delete'} type={'button'} onclickAction={() => deleteMovie(movie.key)}/>
          </Fragment>
        ))}
          <Modal
              isOpen={modalIsOpen}
